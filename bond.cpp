...
double dp(ll missions, ll bonds) {
    if (missions < 0) return 1;
    if (memo[missions][bonds] != -1) return memo[missions][bonds];
    double &ans = memo[missions][bonds] = 0;
    rep(i, 0, 20) {
        if (bonds >> i & 1) {
            ans = max(ans, p[missions][i] * dp(missions - 1, bonds ^ (1 << i)));
        }
    }
    return ans;
}
