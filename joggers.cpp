...
constexpr ll maxn = 50'001;

ll n, s;
bool vis[maxn];
vo<array<ll, 2>> edges[maxn];
vi children[maxn];
void dfs(ll i, ll d) {
    if (d * 2 >= s) return;
    vis[i] = true;
    for (auto [nx, w]: edges[i]) {
        if (!vis[nx]) {
            children[i].push_back(nx);
            dfs(nx, d + w);
        }
    }
}
bool already_covered[maxn];

array<ll, 2> memo[maxn];

array<ll, 2> mvc(ll x) {
    if (memo[x] != array<ll, 2>{-1, -1}) return memo[x];

    ll covered = 1 - already_covered[x];
    for (auto child: children[x]) {
        auto [cov, not_cov] = mvc(child);
        covered += min(cov, not_cov);
    }
    ...
    return memo[x] = {covered, not_covered};
}
...
int main() {
    ...
    rep(i, 0, l) {
        ...
        read(l_i);
        --l_i;
        already_covered[l_i] = true;
    }
    println(min(mvc(0)[0], mvc(0)[1]));
}
