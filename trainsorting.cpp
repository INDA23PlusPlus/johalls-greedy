...
int main() {
    ...
    // train on its own is always allowed, start at 1
    vi incr(n, 1), decr(n, 1);
    ll ans = 0;

    // for all the trains (reverse so we dont overlap)
    per(i, n, 0) {
        // for all the trains that come after
        rep(j, i + 1, n) {
            // can we add it before i?
            if (v[j] < v[i] && decr[i] < decr[j] + 1) decr[i] = decr[j] + 1;

            // can we add it after i?
            if (v[i] < v[j] && incr[i] < incr[j] + 1) incr[i] = incr[j] + 1;
        }

        // if this car is in the middle, how long will the whole train be?
        // get max of this over all i
        if (ans < incr[i] + decr[i] - 1)
            ans = incr[i] + decr[i] - 1;
    }
    println(ans);
}
