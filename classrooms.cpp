...
int main() {
    ...
    vo<array<ll, 2>> activities(n);
    read(activities);
    for (auto &[s, e]: activities) e++;
    for (auto &[a, b]: activities) swap(a, b);
    sort(all(activities)); // sort by end times, always want to take the activity that ends the soonest
    for (auto &[a, b]: activities) swap(a, b);
    dbg(activities);

    ll ans = 0;
    multiset<ll, greater<>> classrooms;
    for (auto [start, end]: activities) {
        auto latest_possible = classrooms.lower_bound(start);

        // if there arent any rooms with activities ending at appropriate times
        if (latest_possible == classrooms.end()) {
            // can we add one more classroom?
            if (classrooms.size() < k) {
                classrooms.insert(end);
                ++ans;
            }
        } else {
            // replace the old activity in the classroom with the new one
            classrooms.erase(latest_possible); 
            classrooms.insert(end);
            ++ans;
        }
    }
