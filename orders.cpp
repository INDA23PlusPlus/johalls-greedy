...
int main() {
    ...
    ll ways_to_reach[31000]{};
    ll prev[31000];
    ways_to_reach[0] = 1;
    // rep(i, 0, 30000) {
    //     if (!ways_to_reach[i]) continue;
    //     rep(j, 0, n) {
    //         ways_to_reach[i + v[j]] += ways_to_reach[i];
    //         prev[i + v[j]] = j;
    //     }
    // }
    ...
    rep(i, 0, m) {
        ll q;
        read(q);
        if (ways_to_reach[q] == 0) {
            println("Impossible");
        } else if (ways_to_reach[q] > 1) {
            println("Ambiguous");
        } else {
            vi order;
            while (q > 0) {
                order.push_back(prev[q] + 1);
                q -= v[prev[q]];
            }
            for (auto i: order) print(i);
            println("");
        }
    }
}
